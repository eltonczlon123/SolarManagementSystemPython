...
def run():
    ...
    while True:
        ...
        if selectedOption == 1:
            tui.started("Data loading operation..")
            ...
            try:
                ...
                tui.completed("Data loading operation")
        ...
        if selectedOption == 2:
            ...
            if opt == 1:
                ...
                for i in range(len(records)):
                    if i == 0:
                        continue
                    if entity == records[i][0]:
                        print(entity)
                        flag = False
                    if flag:
                        print('Your entity is not found in records')
                    tui.completed('Entity retrieval process')
                    if opt == 2:
                        tui.started('Entity details retrieval process')
                        value = tui.entity_name()

                        def chkIndex(lst, val):
                            for i in range(len(lst)):
                                for j in range(len(lst[i])):
                                    if val == records[i][j]:
                                        return (i)

                        chk = any(value in sublist for sublist in records)
                        if chk:
                            indx = chkIndex(records, value)
                            lst = []
                            lst = [records[indx]]
                            tui.list_entity(lst, [])
                        else:
                            print('Entity is not found in records]\n')
                        tui.completed('Entity details retrieval process')

                    if opt == 3:
                        tui.started('Type categorisation process')
                        planet = []
                        nonPlanet = []
                        for i in range(len(records)):
                            if i == 0:
                                continue
                            if records[i][1] == 'TRUE':
                                planet.append(records[1][0])
                            else:
                                nonPlanet.append(records[i][0])
                        dictionary = {
                            "planets": planet,
                            "non-planets": nonPlanet}
                        tui.list_categories(dictionary)
                        tui.completed

                    if opt == 4:
                        tui.started('Categorisation by entity gravity process')
                        GravityRange = tui.gravity_range
                        lwGrvt = GravityRange[0]
                        hghGrvt = GravityRange[1]
                        LowGravity = []
                        medGravity = []
                        highGravity = []
                        for i in range(len(records)):
                            if i == 0:
                                if float(records[i][8]) < lwGrvt:
                                    LowGravity.append(records[1][0])
                                elif float(records[i][8]) > hghGrvt:
                                    highGravity.append(records[i][0])
                                else:
                                    medGravity.append(records[i][0])
                        dict = {
                            "low": LowGravity, "medium": medGravity,
                            "high": highGravity}
                        tui.list_categories(dict)
                        tui.completed('Categorisation by entity gravity process')
                    if opt == 5:
                        tui.started('Orbit summary process')
                        orbitsList = tui.orbits()
                        orbitedEntities = []
                        LowOrbitedEntitiesCategory = []
                        HighOrbitedEntitiesCategory = []
                        for i in range(len(records)):
                            if i == 0:
                                continue
                            if (records[i][21]) in orbitsList:
                                orbitedEntities.append(records[1][0])
                                if float(records[1][10]) < 100:
                                    LowOrbitedEntitiesCategory.append(records[i][0])
                                else:
                                    HighOrbitedEntitiesCategory.append(records[i][0])
                    orbit_summary = {
                        "planet_orbited": {
                            "low": LowOrbitedEntitiesCategory,
                            "high": HighOrbitedEntitiesCategory
                        }
                    }
                    tui.list_categories(orbit_summary)
